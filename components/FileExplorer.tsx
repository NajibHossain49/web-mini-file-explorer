...
import { HardDrive, RotateCcw, Menu, X } from "lucide-react";

export default function FileExplorer() {
  const {
    state, selectedFolder, openFile,
    createNode, renameNode, deleteNode, selectFolder,
    openFileAction, closeFile, saveFile, setEditingContent,
    toggleExpand, resetToDefault,
  } = useFileSystem();
  ...
  const createParentNode = createParentId
    ? (state.root.id === createParentId ? state.root : selectedFolder)
    : null;

  return (
    <div style={{ height: "100dvh", display: "flex", flexDirection: "column", background: "#11111b", color: "#cdd6f4" }}>
      {/* Title Bar */}
      <header style={{
        display: "flex",
        alignItems: "center",
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
          <button
            ...
            style={{ display: "none", alignItems: "center", justifyContent: "center", width: "32px", height: "32px", borderRadius: "8px", border: "none", background: "transparent", color: "#6c7086", cursor: "pointer", transition: "all 0.12s" }}
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
            ...
            <div>
              <p style={{ fontSize: "14px", fontWeight: 700, color: "#cdd6f4", letterSpacing: "-0.03em", lineHeight: 1 }}>
                File Explorer
              </p>
              <p style={{ fontSize: "10px", color: "#6c7086", marginTop: "2px", lineHeight: 1 }}>
                Webbly Media
              </p>
            </div>
          </div>
      ...
      <div style={{ flex: 1, display: "flex", minHeight: 0, position: "relative" }}>
        ...
        <div style={{
          ...
          <Sidebar
            ...
            onSelectFolder={(id) => { selectFolder(id); setSidebarOpen(false); }}
            onOpenFile={(node) => { openFileAction(node); setSidebarOpen(false); }}
